    ...
    public class CosmicHorrorPawn : Pawn
    {
                ...
                if (this.def == MonsterDefOf.ROM_Chthonian.race)
                {
                    ...
                    if (this.lastSpot != IntVec3.Invalid)
                    {
                        ...
                        if (this.lastSpot != this.Position)
                        {
                            ...
                            {
                                //Throw some smoke
                                FleckMaker.ThrowSmoke(intVec.ToVector3(), map, 1f);
                                FleckMaker.ThrowDustPuff(intVec, map, 1f);
                                FleckMaker.ThrowDustPuff(this.lastSpot, map, 0.5f);
        ...
        public void Reveal()
        {
            if (this.IsInvisible)
            {
                ...
                FleckMaker.ThrowAirPuffUp(GenThing.TrueCenter(this), this.MapHeld);
            }
            return;
        }
